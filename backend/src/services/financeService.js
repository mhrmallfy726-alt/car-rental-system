...
const getDashboard = async () => {
  const [summary, adPayments, reservationPayments, pendingPayouts] = await Promise.all([
    query(`SELECT COALESCE(SUM(CASE WHEN status IN ('paid','partially_refunded') THEN amount-COALESCE(refund_amount,0) ELSE 0 END),0)::numeric AS gross_revenue,COALESCE(SUM(CASE WHEN status IN ('paid','partially_refunded') AND advertisement_id IS NOT NULL THEN amount-COALESCE(refund_amount,0) ELSE 0 END),0)::numeric AS advertisement_revenue,COALESCE(SUM(CASE WHEN status IN ('paid','partially_refunded') AND reservation_id IS NOT NULL THEN amount-COALESCE(refund_amount,0) ELSE 0 END),0)::numeric AS reservation_revenue,COALESCE((SELECT SUM(amount) FILTER (WHERE direction='credit') - SUM(amount) FILTER (WHERE direction='debit') FROM ledger_entries WHERE entry_type='platform_fee'),0)::numeric AS platform_commission,COALESCE((SELECT SUM(amount) FILTER (WHERE direction='credit') - SUM(amount) FILTER (WHERE direction='debit') FROM ledger_entries WHERE entry_type='supplier_payable'),0)::numeric AS supplier_payable,COALESCE(SUM(CASE WHEN status IN ('refunded','partially_refunded') THEN COALESCE(refund_amount,0) ELSE 0 END),0)::numeric AS refunded_amount,COUNT(*) FILTER (WHERE status='paid')::int AS paid_transactions,COUNT(*) FILTER (WHERE status='pending')::int AS pending_transactions FROM payments`),
    ...
    query(`SELECT p.id,p.reservation_id,p.amount,p.currency,p.status,p.refund_amount,p.paid_at,p.provider_reference,r.start_date,r.end_date,r.with_driver,c.make,c.model,su.name AS supplier_name,cu.name AS customer_name,COALESCE(fee.amount,0)::numeric AS commission,CASE WHEN p.amount > 0 THEN ROUND((COALESCE(fee.amount,0)/p.amount*100)::numeric,2) ELSE 0 END AS commission_rate,COALESCE(payable.amount,0)::numeric AS supplier_amount,COALESCE(pending.amount,0)::numeric AS pending_amount,GREATEST(0,p.amount-COALESCE(p.refund_amount,0))::numeric AS net_amount,CASE WHEN p.status IN ('paid','partially_refunded') AND ((COALESCE(pending.amount,0)>0 AND COALESCE(fee.amount,0)=0 AND COALESCE(payable.amount,0)=0 AND ABS(COALESCE(pending.amount,0)-(GREATEST(0,p.amount-COALESCE(p.refund_amount,0))*(1-COALESCE(NULLIF(p.metadata->>'commission_rate','')::numeric,0)/100)))<0.01) OR (COALESCE(fee.amount,0)+COALESCE(payable.amount,0)=GREATEST(0,p.amount-COALESCE(p.refund_amount,0)) AND COALESCE(pending.amount,0)=0 AND ABS(COALESCE(fee.amount,0)+COALESCE(payable.amount,0)-GREATEST(0,p.amount-COALESCE(p.refund_amount,0)))<0.01)) THEN 'matched' WHEN p.status='refunded' AND ABS(COALESCE(fee.amount,0)+COALESCE(payable.amount,0)+COALESCE(pending.amount,0))<0.01 THEN 'matched' ELSE 'check' END AS reconciliation_status FROM payments p LEFT JOIN reservations r ON r.id=p.reservation_id LEFT JOIN cars c ON c.id=r.car_id LEFT JOIN users su ON su.id=r.supplier_id LEFT JOIN users cu ON cu.id=r.customer_id LEFT JOIN LATERAL (SELECT COALESCE(SUM(amount) FILTER (WHERE direction='credit')-SUM(amount) FILTER (WHERE direction='debit'),0) AS amount FROM ledger_entries WHERE payment_id=p.id AND entry_type='platform_fee') fee ON TRUE LEFT JOIN LATERAL (SELECT COALESCE(SUM(amount) FILTER (WHERE direction='credit')-SUM(amount) FILTER (WHERE direction='debit'),0) AS amount FROM ledger_entries WHERE payment_id=p.id AND entry_type='supplier_payable') payable ON TRUE LEFT JOIN LATERAL (SELECT COALESCE(SUM(amount) FILTER (WHERE direction='credit')-SUM(amount) FILTER (WHERE direction='debit'),0) AS amount FROM ledger_entries WHERE payment_id=p.id AND entry_type='supplier_pending') pending ON TRUE WHERE p.reservation_id IS NOT NULL ORDER BY p.created_at DESC LIMIT 100`),
