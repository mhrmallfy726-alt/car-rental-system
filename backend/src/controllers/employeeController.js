...
// // Helpers
// // ========================
// const ensureSupplierScope = (reqSupplierId, req) => {
//   // إذا كان المستخدم موردًا، تأكد أن المورد الموجّه له هو نفس المستخدَم في التوكن
//   if (req.role === 'supplier' && req.id !== reqSupplierId) {
//     return false;
//   }
//   return true;
// };
...
//     if (!supplier_id) {
//       return res.status(400).json({ success: false, message: 'supplier_id مطلوب' });
//     }

//     if (!ensureSupplierScope(supplier_id, req.)) {
...
//     if (!ensureSupplierScope(emp.supplier_id, req.)) {
//       return res.status(403).json({ success: false, message: 'غير مصرح' });
//     }
...
//     // تحقق إن كان البريد موجود مسبقاً
...
// // body: fields to update (full_name, phone_number, role, status)
// const updateEmployee = async (req, res) => {
//   try {
//     const id = req.params.id;
//     const { full_name, phone_number, role, status } = req.body;

//     // تحقق وجود الموظف + صلاحية المورد
...
//     if (!ensureSupplierScope(supplier_id, req.)) return res.status(403).json({ success: false, message: 'غير مصرح' });

//     // بناء جملة UPDATE مرنة
//     const sets = [];
//     const vals = [];
//     let idx = 1;
...
// // body: { permission_ids: [1,2,3] }   — تعيين مجموعة صلاحيات جديدة (يستبدل)
...
// // GET /api/employees/permissions/list  -> قائمة كل الصلاحيات المتاحة
