...
function CustomerProfile({ row }) {
  ...
  const bookings = row.reservations || [];
  return <details style={styles.customerDetails}>
    <summary style={styles.customerSummary}>عرض حجوزات العميل وتفضيلاته</summary>
    <div style={styles.customerGrid}>
      <div><span>الهاتف</span><strong>{row.phone || 'غير مسجل'}</strong></div>
      <div><span>عدد الحجوزات</span><strong>{row.reservations_count || 0}</strong></div>
      <div><span>إجمالي قيمة الحجوزات</span><strong>{Number(row.total_booking_value || 0).toLocaleString()} ر.ي.</strong></div>
      <div><span>التفضيل الحالي</span><strong>{preferences.preferred_service || 'بدون سائق'}</strong></div>
      <div><span>آخر موقع تسليم</span><strong>{preferences.last_pickup_location || 'غير محدد'}</strong></div>
      <div><span>آخر موقع إرجاع</span><strong>{preferences.last_dropoff_location || 'غير محدد'}</strong></div>
    </div>
    <div style={styles.customerBookingList}>
      <strong>سجل الحجوزات</strong>
      {bookings.length ? bookings.slice(0, 8).map((booking) => <div key={booking.id} style={styles.customerBooking}>
        <span>{booking.car?.make} {booking.car?.model} · {booking.start_date} إلى {booking.end_date}</span>
        <small>{booking.status} · {booking.with_driver ? 'مع سائق' : 'بدون سائق'} · {Number(booking.total_price || 0).toLocaleString()} ر.ي.</small>
      </div>) : <small>لا توجد حجوزات.</small>}
    </div>
    {!!preferences.favorite_cars?.length && <div style={styles.customerFavorites}><strong>السيارات المفضلة</strong><span>{preferences.favorite_cars.map((car) => `${car.make} ${car.model}`).join('، ')}</span></div>}
  </details>;
}
