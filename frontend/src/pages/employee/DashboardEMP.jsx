...
import {
  ...
  Users, WalletCards, XCircle, Menu,
} from 'lucide-react';
...
export default function EmployeeDashboard() {
  ...
  return (
    <main className={`employee-shell ${mobileMenuOpen ? 'employee-mobile-menu-open' : ''}`} dir="rtl">
      <style>{`
        .employee-shell{min-height:100vh;background:#f5f7fb;color:#173a52;padding:28px}.employee-mobile-menu-button{display:none;border:1px solid #dfe8ed;background:#fff;color:#173a52;padding:10px 13px;border-radius:12px;font:inherit;font-weight:800;cursor:pointer;align-items:center;gap:7px}
      ...
      <div className="employee-wrap">
        ...
        <section className="employee-content">
          <header className="employee-top">
            <button className="employee-mobile-menu-button" type="button" onClick={() => setMobileMenuOpen((value) => !value)} aria-expanded={mobileMenuOpen}><Menu size={18} /> {mobileMenuOpen ? 'إغلاق القائمة' : 'قائمة العمل'}</button>
