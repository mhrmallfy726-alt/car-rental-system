...
export default function Login() {
  ...
  const supplierFileInputs = useRef({});
  ...
  const handleResubmitDocuments = async () => {
    // اقرأ من عناصر input نفسها أيضا لضمان عدم فقدان الملف عند إعادة رسم نافذة الرفض.
    const selectedFiles = Object.keys(supplierFiles)
      .map((field) => [field, supplierFileInputs.current[field]?.files?.[0] || supplierFiles[field]])
    ...
    try {
      ...
      Object.values(supplierFileInputs.current).forEach((input) => {
          ...
          {supplierStatus.commercial_register_reason && (
            <div className="mb-5 border rounded-xl p-4 bg-red-50">
              ...
              <input
                ...
                ref={(input) => { supplierFileInputs.current.commercial_register = input; }}
          ...
          {supplierStatus.owner_id_reason && (
            <div className="mb-5 border rounded-xl p-4 bg-red-50">
              ...
              <input
                ...
                name="owner_id"
                accept="image/*,.pdf"
                ref={(input) => { supplierFileInputs.current.owner_id = input; }}
                onChange={handleSupplierFileChange}
                className="mt-3 w-full rounded-lg border border-gray-200 bg-white p-2 text-sm"
              />
          ...
          {supplierStatus.avatar_reason && (
            <div className="mb-5 border rounded-xl p-4 bg-red-50">
              ...
              <input
                ...
                name="avatar"
                accept="image/*"
                ref={(input) => { supplierFileInputs.current.avatar = input; }}
                onChange={handleSupplierFileChange}
                className="mt-3 w-full rounded-lg border border-gray-200 bg-white p-2 text-sm"
              />
