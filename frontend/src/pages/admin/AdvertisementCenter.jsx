    ...
    <main dir="rtl" style={{ minHeight: '100vh', background: '#f7f8fa' }}>
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        <aside className="sidebar" style={{ width: 248, flexShrink: 0, padding: '26px 15px', background: '#fff', borderLeft: '1px solid #e8edf0', position: 'sticky', top: 0, height: '100vh', boxSizing: 'border-box' }}>
          ...
          {[['/admin/dashboard', LayoutDashboard, 'لوحة الإحصائيات'], ['/admin/advertisements', Megaphone, 'الإعلانات'], ['/admin/supplier-requests', Users, 'طلبات الموردين'], ['/admin/cars', ShieldAlert, 'السيارات'], ['/admin/settings', Settings, 'الإعدادات']].map(([to, Icon, label]) => <Link key={to} to={to} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 13px', marginBottom: 4, borderRadius: 11, color: to === '/admin/advertisements' ? navy : '#4d5a64', background: to === '/admin/advertisements' ? '#eef4f6' : 'transparent', textDecoration: 'none', fontWeight: to === '/admin/advertisements' ? 900 : 700, fontSize: 14 }}><Icon size={18} />{label}</Link>)}
        </aside>
