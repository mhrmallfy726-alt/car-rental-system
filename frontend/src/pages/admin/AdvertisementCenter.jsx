...
import { adminAPI, advertisementsAPI } from '../../services/api';
...
const blankForm = { supplier_id: '', car_id: '', title: '', description: '', ad_type: 'featured', placement: 'cars', image_url: '', link_url: '', duration_days: 7, start_date: '', end_date: '', status: 'draft', featured: false, is_pinned: false };

export default function AdvertisementCenter() {
  const [tab, setTab] = useState('overview');
  const [ads, setAds] = useState([]);
  const [requests, setRequests] = useState([]);
  const [stats, setStats] = useState({ total_ads: 0, active_ads: 0, pending_ads: 0, inactive_ads: 0, impressions: 0, clicks: 0, total_requests: 0, pending_requests: 0, approved_requests: 0, rejected_requests: 0, total_ad_value: 0 });
  ...
  const loadData = async () => {
    ...
    const [adsResponse, requestsResponse, statsResponse, suppliersResponse, pricingResponse] = results;
    ...
    if (pricingResponse.status === 'fulfilled') {
      setPricing(pricingResponse.value.data?.data || null);
    }
  ...
  const updateDuration = (event) => {
    const duration_days = Number(event.target.value || 0);
    setForm((current) => ({ ...current, duration_days, end_date: calculateEndDate(current.start_date, duration_days) }));
  };
  const updateStartDate = (nextValue) => {
    setForm((current) => ({ ...current, start_date: nextValue, end_date: calculateEndDate(nextValue, current.duration_days) }));
  };
  ...
  const getPlacementPrice = (placement) => {
    if (!pricing) return 0;
    const base = Number(pricing.advertisement_price_per_day || 0);
    return Number(({ home: pricing.advertisement_price_home_per_day, cars: pricing.advertisement_price_cars_per_day, car_detail: pricing.advertisement_price_car_detail_per_day, all_public: pricing.advertisement_price_all_public_per_day })[placement] ?? base);
  };
  const calculateTotal = (placement, duration) => getPlacementPrice(placement) * Number(duration || 0);

  const editAdvertisement = async (ad) => {
    setEditingId(ad.id);
    setImageFile(null);
    setImagePreview(ad.image_url || '');
    setForm({ supplier_id: ad.supplier_id || '', car_id: ad.car_id || '', title: ad.title || '', description: ad.description || '', ad_type: ad.ad_type || 'featured', placement: ad.placement || 'cars', image_url: ad.image_url || '', link_url: ad.link_url || '', duration_days: ad.duration_days || 7, start_date: ad.start_date || '', end_date: ad.end_date || '', status: ad.status || 'draft', featured: ad.featured || false, is_pinned: ad.is_pinned || false });
    if (ad.supplier_id) {
...
function Field({ label, name, type = 'text', value, onChange, options, placeholder, required, wide, readOnly }) { return <label style={{ gridColumn: wide ? '1 / -1' : undefined, color: navy, fontSize: 13, fontWeight: 800 }}>{label}{type === 'select' ? <select name={name} value={value} onChange={onChange} style={inputStyle} required={required}>{options.map(([key, optionLabel]) => <option value={key} key={key}>{optionLabel}</option>)}</select> : type === 'date' ? <UnifiedDatePicker value={value} onChange={(nextValue) => onChange({ target: { name, value: nextValue } })} placeholder={`اختر ${label}`} isClearable={false} /> : type === 'textarea' ? <textarea name={name} value={value} onChange={onChange} style={{ ...inputStyle, minHeight: 95, resize: 'vertical' }} required={required} placeholder={placeholder} /> : <input name={name} value={value} onChange={onChange} type={type} style={inputStyle} required={required} placeholder={placeholder} readOnly={readOnly} />}</label>; }
