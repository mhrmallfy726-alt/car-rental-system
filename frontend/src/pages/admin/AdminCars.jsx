...
export default function AdminCars() {
  ...
  const approvedCount = cars.length - pendingCount;
  ...
  return <main className="admin-cars-page" dir="rtl">
    <div className="admin-cars-shell"><AdminSidebar /><section className="admin-cars-content">
      <header className="admin-cars-header"><div><span className="admin-cars-kicker">إدارة الأسطول</span><h1>إدارة السيارات</h1><p>تُعتمد السيارات الجديدة تلقائيًا، ويمكن مراجعة الحالات الاستثنائية من هنا.</p></div><div className="admin-cars-header-actions"><span className="admin-pending-count">{pendingCount} حالات تحتاج مراجعة</span><button type="button" onClick={fetchCars} disabled={loading}><RefreshCw size={15} className={loading ? 'spin' : ''} /> تحديث</button></div></header>
      <section className="admin-cars-overview" aria-label="ملخص السيارات"><div className="admin-overview-card total"><span className="admin-overview-icon"><Car size={19} /></span><div><small>إجمالي السيارات</small><strong>{cars.length}</strong><em>في أسطول المنصة</em></div></div><div className="admin-overview-card approved"><span className="admin-overview-icon"><CheckCircle size={19} /></span><div><small>السيارات المفعّلة</small><strong>{approvedCount}</strong><em>متاحة للعرض</em></div></div><div className="admin-overview-card pending"><span className="admin-overview-icon"><Clock size={19} /></span><div><small>بانتظار الموافقة</small><strong>{pendingCount}</strong><em>تحتاج مراجعة</em></div></div></section>
      <nav className="admin-cars-filters" aria-label="تصفية السيارات"><span className="admin-filter-label">عرض السيارات</span>{[['all', 'الكل'], ['pending', 'بانتظار الموافقة'], ['approved', 'مفعّلة']].map(([value, label]) => <button type="button" key={value} onClick={() => setFilter(value)} className={filter === value ? 'active' : ''}>{label}{value === 'pending' && ` (${pendingCount})`}</button>)}</nav>
    ...
    <style>{styles}</style><style>{`@media(min-width:901px){.admin-cars-table .table-car-img,.admin-cars-table .table-car-placeholder{width:92px;height:58px;border-radius:10px;box-shadow:0 4px 10px rgba(23,58,82,.08)}}`}</style>
  </main>;
}
...
const styles = `
.admin-cars-page{min-height:100vh;padding:34px 22px 70px;background:linear-gradient(135deg,#f4f8f9,#edf5f5);color:#173a52}.admin-cars-shell{display:flex;min-height:calc(100vh - 104px)}.admin-cars-content{flex:1;min-width:0;padding:0 22px 0 0}.admin-cars-header{display:flex;justify-content:space-between;align-items:flex-end;gap:18px;max-width:1200px;margin:0 auto 22px;padding:2px 4px}.admin-cars-kicker{color:#0b8a73;font-size:10px;font-weight:950;letter-spacing:.5px}.admin-cars-header h1{margin:6px 0;color:#173a52;font-size:30px;letter-spacing:-.5px}.admin-cars-header p{margin:0;color:#78909b;font-size:13px}.admin-cars-header-actions{display:flex;align-items:center;gap:9px}.admin-pending-count{padding:8px 12px;border-radius:999px;background:#fff0bd;color:#745516;font-size:11px;font-weight:900}.admin-cars-header-actions button,.admin-cars-state button{display:inline-flex;align-items:center;gap:6px;border:1px solid #d3e1e4;border-radius:10px;padding:9px 12px;background:#fff;color:#526873;font:inherit;font-size:11px;font-weight:850;cursor:pointer;box-shadow:0 4px 12px rgba(23,58,82,.04)}.admin-cars-header-actions button:disabled{opacity:.6}.admin-cars-overview{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;max-width:1200px;margin:0 auto 20px}.admin-overview-card{display:flex;align-items:center;gap:13px;padding:17px 18px;border:1px solid #dce8ea;border-radius:17px;background:#fff;box-shadow:0 8px 20px rgba(23,58,82,.055)}.admin-overview-icon{display:grid;place-items:center;width:42px;height:42px;border-radius:13px}.admin-overview-card.total .admin-overview-icon{background:#eaf2ff;color:#3468bd}.admin-overview-card.approved .admin-overview-icon{background:#e4fbf5;color:#087f68}.admin-overview-card.pending .admin-overview-icon{background:#fff4d2;color:#a2771d}.admin-overview-card div{display:grid;gap:3px}.admin-overview-card small{color:#8498a1;font-size:10px;font-weight:800}.admin-overview-card strong{color:#173a52;font-size:23px;line-height:1}.admin-overview-card em{color:#9aa9ae;font-size:9px;font-style:normal}.admin-cars-filters{display:flex;align-items:center;gap:8px;max-width:1200px;margin:0 auto 14px}.admin-filter-label{margin-left:auto;color:#8498a1;font-size:11px;font-weight:850}.admin-cars-filters button{border:1px solid #dce7eb;border-radius:10px;padding:9px 13px;background:#fff;color:#526873;font:inherit;font-size:11px;font-weight:850;cursor:pointer}.admin-cars-filters button.active{border-color:#173a52;background:#173a52;color:#fff;box-shadow:0 6px 12px rgba(23,58,82,.14)}.admin-cars-table-wrap{max-width:1200px;margin:0 auto;overflow-x:auto;border:1px solid #dce7eb;border-radius:18px;background:#fff;box-shadow:0 12px 28px rgba(23,58,82,.07)}.admin-cars-table{width:100%;border-collapse:collapse;min-width:760px;font-size:12px}.admin-cars-table th{padding:15px 16px;background:#f7fafb;color:#8498a1;text-align:right;font-size:10px;font-weight:900}.admin-cars-table td{padding:14px 16px;border-top:1px solid #edf2f4;color:#526873}.admin-cars-table tbody tr{transition:background .18s}.admin-cars-table tbody tr:hover{background:#fbfefe}.admin-cars-table .car-name{color:#173a52;font-weight:900}.car-name small{display:block;margin-top:3px;color:#91a0a7;font-weight:600}.price{color:#087f68!important;font-weight:900}.table-car-img,.table-car-placeholder{width:68px;height:48px;display:grid;place-items:center;object-fit:cover;border-radius:9px;background:#edf3f5;color:#91a2aa}.status{display:inline-flex;align-items:center;gap:5px;padding:6px 9px;border-radius:999px;font-size:10px;font-weight:900;white-space:nowrap}.status.approved{background:#e4fbf5;color:#087f68}.status.pending{background:#fff0bd;color:#745516}.details-link,.approve-table{display:inline-flex;align-items:center;gap:5px;margin-left:7px;border:0;border-radius:8px;padding:7px 9px;text-decoration:none;font:inherit;font-size:10px;font-weight:900;cursor:pointer}.details-link{background:#eef5ff;color:#087f68}.approve-table{background:#e4fbf5;color:#087f68}.approve-table:disabled{opacity:.6}.admin-mobile-car-cards{display:none}.admin-cars-state{display:grid;place-items:center;align-content:center;gap:10px;min-height:360px;max-width:1200px;margin:auto;color:#91a0a7;text-align:center}.admin-cars-state h2{margin:0;color:#173a52;font-size:18px}.admin-cars-state p{margin:0;font-size:12px}.admin-cars-state.error{padding:30px}.admin-cars-state.error svg{color:#a23a3a}.spin{animation:admin-spin .8s linear infinite}@keyframes admin-spin{to{transform:rotate(360deg)}}.admin-loader{width:36px;height:36px;border:4px solid #dfeaec;border-top-color:#0b8a73;border-radius:50%;animation:admin-spin .8s linear infinite}
@media(max-width:900px){.admin-cars-page{padding:132px 12px 50px}.admin-cars-shell{display:block;min-height:auto}.admin-cars-content{padding:0}.admin-cars-header{align-items:stretch;flex-direction:column;margin-bottom:15px}.admin-cars-header h1{font-size:25px}.admin-cars-header-actions{justify-content:space-between}.admin-cars-header-actions button{padding-inline:14px}.admin-cars-filters{overflow-x:auto;margin-bottom:14px}.admin-cars-filters button{flex:0 0 auto}.admin-cars-table-wrap{display:none}.admin-mobile-car-cards{display:grid;grid-template-columns:1fr;gap:14px}.admin-mobile-car-card{overflow:hidden;border:1px solid #dce9ed;border-radius:18px;background:#fff;box-shadow:0 9px 24px rgba(23,58,82,.07)}.admin-mobile-car-image{position:relative;height:180px;background:#edf3f5}.large-car-img{width:100%;height:100%;display:block;object-fit:cover}.large-car-placeholder{width:100%;height:100%;display:grid;place-items:center;color:#91a2aa}.pending-badge{position:absolute;top:12px;right:12px;display:inline-flex;align-items:center;gap:5px;padding:7px 10px;border-radius:999px;background:#fff0bd;color:#745516;font-size:10px;font-weight:900}.admin-mobile-car-body{padding:15px}.admin-mobile-car-title{display:flex;justify-content:space-between;align-items:flex-start;gap:10px}.admin-mobile-car-title h2{margin:0 0 5px;color:#173a52;font-size:18px}.admin-mobile-car-title small{color:#83959d;font-size:11px}.admin-mobile-car-facts{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:15px 0 12px;padding:12px 0;border-top:1px solid #edf2f4;border-bottom:1px solid #edf2f4}.admin-mobile-car-facts span{display:grid;gap:4px;color:#526873;font-size:11px}.admin-mobile-car-facts b{color:#93a1a7;font-size:9px}.admin-mobile-car-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.admin-mobile-car-actions button{display:inline-flex;justify-content:center;align-items:center;gap:6px;border:1px solid #dce7eb;border-radius:10px;padding:10px;background:#fff;color:#173a52;font:inherit;font-size:11px;font-weight:900;cursor:pointer}.admin-mobile-car-actions button.approve{border-color:#bfe9dc;background:#e4fbf5;color:#087f68}.admin-mobile-car-actions button:disabled{opacity:.6}}@media(max-width:460px){.admin-mobile-car-image{height:165px}.admin-mobile-car-facts{grid-template-columns:repeat(3,1fr)}.admin-mobile-car-title{display:block}.admin-mobile-car-title .status{margin-top:10px}}
`;
