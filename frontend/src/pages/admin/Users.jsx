...
export default function AdminUsers() {
  ...
  const getRoleBadge = (role) => {
    ...
    const colors = { admin: '#087f68', supplier: '#24647d', customer: '#6c757d' };
  ...
  return <div dir="rtl" className="admin-users-page">
    ...
    <style>{`
      .admin-users-page{display:flex;min-height:100vh;background:#f7fafb;color:#18212b}
      ...
      .users-summary{background:#fff;border:1px solid #e7ebef;border-radius:12px;padding:10px 16px;text-align:center;min-width:72px;box-shadow:0 2px 8px #18212b0b}.users-summary strong{display:block;color:#087f68;font-size:1.3rem}.users-summary span{font-size:.75rem;color:#6c757d}
      .desktop-users-table{width:100%;overflow-x:auto;border-radius:14px;box-shadow:0 2px 12px #18212b0b;background:#fff}.users-table{width:100%;min-width:780px;border-collapse:collapse}.users-table th{background:#f5f7f9;color:#53606c;font-size:.82rem;font-weight:700;white-space:nowrap}.users-table th,.users-table td{padding:14px 16px;text-align:right;border-bottom:1px solid #edf0f2;vertical-align:middle}.users-table tr:last-child td{border-bottom:0}.user-name-cell{font-weight:700}.email-cell{direction:ltr;text-align:right;color:#5e6973;font-size:.88rem}.role-badge,.status-badge{display:inline-flex;align-items:center;justify-content:center;white-space:nowrap;padding:4px 9px;border-radius:999px;color:#fff;font-size:.72rem;font-weight:700}.status-active{background:#0f766e}.status-blocked{background:#b42318}.violation-count{color:#6c757d;font-weight:800}.has-violations{color:#b42318}.user-actions{display:flex;gap:7px;flex-wrap:wrap}.action-button{border:0;border-radius:7px;padding:8px 10px;display:inline-flex;align-items:center;justify-content:center;gap:5px;font-size:.76rem;font-weight:700;cursor:pointer;white-space:nowrap}.action-button:disabled{opacity:.6;cursor:not-allowed}.violation-button{background:#b78a22;color:#212529}.disable-button{background:#b42318;color:#fff}.enable-button{background:#0f766e;color:#fff}.mobile-users-list{display:none}.empty-users{background:#fff;border-radius:14px;padding:42px;text-align:center;color:#6c757d;box-shadow:0 2px 12px #18212b0b}
      .violation-modal-backdrop{position:fixed;inset:0;background:#10182099;display:grid;place-items:center;z-index:20;padding:clamp(12px,4vw,28px);overflow-y:auto}.violation-modal{background:#fff;border-radius:16px;padding:clamp(20px,4vw,30px);width:min(520px,100%);position:relative;box-shadow:0 18px 60px #0004}.violation-modal h2{margin:0 0 8px;font-size:1.35rem}.modal-user-label{color:#5e6973;margin:0 0 22px;font-size:.9rem}.close-modal-button{position:absolute;left:16px;top:16px;border:0;background:#f1f3f5;color:#495057;width:34px;height:34px;border-radius:50%;display:grid;place-items:center;cursor:pointer}.violation-modal label{display:block;font-weight:700;font-size:.85rem;margin:14px 0 7px}.violation-modal input,.violation-modal textarea{display:block;width:100%;box-sizing:border-box;border:1px solid #d8dee4;border-radius:8px;padding:11px;font:inherit;outline:none}.violation-modal input:focus,.violation-modal textarea:focus{border-color:#087f68;box-shadow:0 0 0 3px #087f681c}.violation-modal textarea{resize:vertical;min-height:100px}.warning-note{background:#fff3cd;color:#856404;border-radius:9px;padding:11px;margin:18px 0;font-size:.83rem;line-height:1.7}.submit-violation-button{width:100%;border:0;border-radius:8px;background:#087f68;color:#fff;padding:12px;font-weight:700;cursor:pointer}.submit-violation-button:disabled{opacity:.6;cursor:not-allowed}.users-loading{display:grid;place-items:center;min-height:300px}.spinner{width:40px;height:40px;border:4px solid #e9ecef;border-top-color:#087f68;border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
