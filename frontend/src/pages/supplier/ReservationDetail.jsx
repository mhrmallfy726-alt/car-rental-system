    ...
    <div className="dashboard" style={{ display: 'flex', minHeight: '100vh', background: '#f7fafb' }}>
      ...
      <div className="dashboard-content" style={{ flex: 1, padding: '30px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
          ...
          <span style={{
            background: ['pending_payment', 'pending'].includes(reservation.status) ? '#b78a22' : ['approved', 'awaiting_pickup'].includes(reservation.status) ? '#24647d' : reservation.status === 'active' ? '#0f766e' : reservation.status === 'returned' ? '#8b5cf6' : reservation.status === 'completed' ? '#087f68' : ['rejected', 'cancelled', 'disputed'].includes(reservation.status) ? '#b42318' : '#6c757d',
            color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', marginLeft: 'auto'
          }}>{reservation.status === 'pending_payment' ? 'بانتظار الدفع' : reservation.status === 'pending' ? 'مدفوع بانتظار المراجعة' : reservation.status === 'approved' ? 'تمت الموافقة — بانتظار الدفع' : reservation.status === 'awaiting_pickup' ? 'بانتظار استلام العميل' : reservation.status === 'active' ? 'السيارة مع العميل' : reservation.status === 'returned' ? 'تم استلام السيارة' : reservation.status === 'completed' ? 'مكتمل' : reservation.status === 'rejected' ? 'مرفوض' : reservation.status === 'cancelled' ? 'ملغي' : reservation.status === 'disputed' ? 'في نزاع' : reservation.status}</span>
