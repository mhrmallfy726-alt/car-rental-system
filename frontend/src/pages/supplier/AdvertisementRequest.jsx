...
const PLACEMENT_LABELS = {
  ...
  all_public: 'كل الصفحات العامة',
};

const getPlacementPrice = (pricing, placement) => {
  if (!pricing) return 0;
...
export default function AdvertisementRequest() {
  ...
  const selectedPricePerDay = useMemo(() => getPlacementPrice(pricing, form.placement), [pricing, form.placement]);
  const selectedTotalPrice = selectedPricePerDay * Number(form.duration_days || 0);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'تعذر تحميل بيانات الإعلانات');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitRequest = async (event) => {
    ...
    if (!form.start_date) return toast.error('اختر تاريخ البداية');
    if (!form.duration_days || Number(form.duration_days) < 1) return toast.error('المدة يجب أن تكون يومًا واحدًا على الأقل');

    const calculatedEndDate = calculateEndDate(form.start_date, form.duration_days);
    if (form.end_date !== calculatedEndDate) return toast.error('تاريخ النهاية لا يطابق المدة المحددة');
    if (parseDate(form.start_date) < today) return toast.error('لا يمكن اختيار تاريخ بداية قديم');
    if (parseDate(form.end_date) < parseDate(form.start_date)) return toast.error('تاريخ النهاية غير صحيح');
    if (!selectedPricePerDay) return toast.error('تعذر تحميل سعر الإعلان من الإدارة');
          ...
          <form onSubmit={submitRequest} style={{ background: '#fff', borderRadius: 20, border: '1px solid #e7eaee', boxShadow: '0 14px 36px rgba(23,58,82,0.07)', padding: 26 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 16 }}>
              ...
              <label style={{ gridColumn: '1 / -1', color: navy, fontWeight: 800, fontSize: 14 }}>صورة الإعلان
                <input name="image" type="file" accept="image/jpeg,image/png,image/webp" onChange={handleImageChange} style={fieldStyle} />
              ...
              <label style={{ color: navy, fontWeight: 800, fontSize: 14 }}>مكان الظهور
                <select name="placement" value={form.placement} onChange={updateField} style={fieldStyle}>
                  {Object.entries(PLACEMENT_LABELS).map(([value, label]) => <option value={value} key={value}>{label} — {getPlacementPrice(pricing, value).toLocaleString()} {pricing?.currency || 'YER'} / يوم</option>)}
                </select>
              </label>
              <label style={{ color: navy, fontWeight: 800, fontSize: 14 }}>سعر الإعلان اليومي
                <input value={pricing ? `${selectedPricePerDay.toLocaleString()} ${pricing.currency || 'YER'}` : 'جاري التحميل...'} readOnly type="text" style={{ ...fieldStyle, background: '#f4f7f9' }} />
              </label>
              <div style={{ color: navy, fontWeight: 800, fontSize: 14, paddingTop: 8 }}>إجمالي الطلب
                <div style={{ ...fieldStyle, marginTop: 8, background: '#fbf7ec', fontSize: 17 }}>{pricing ? `${selectedTotalPrice.toLocaleString()} ${pricing.currency || 'YER'}` : 'جاري الحساب...'}</div>
              </div>
...
const fieldStyle = {
  display: 'block', width: '100%', boxSizing: 'border-box', marginTop: 8, padding: '12px 13px',
  borderRadius: 11, border: '1px solid #dfe5e9', background: '#fff', color: '#243746', font: 'inherit', outline: 'none',
};
