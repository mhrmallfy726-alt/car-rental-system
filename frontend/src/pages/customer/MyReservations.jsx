...
export default function MyReservations() {
  ...
  const handleCancel = async (id) => {
    const reservation = reservations.find((item) => item.id === id);
    const policy = reservation ? getCancellationPolicy(reservation) : null;
    if (!window.confirm(`سياسة الإلغاء: استرداد ${policy?.refund ?? 0}% وخصم ${policy?.fee ?? 100}%. هل تريد المتابعة؟`)) return;
    try {
      await reservationsAPI.cancel(id, { cancellation_reason: 'تم الإلغاء من قبل العميل' });
      toast.success('تم إلغاء الحجز');
      fetchReservations();
  ...
  return (
    <div className="customer-reservations-page" dir="rtl" style={{ background: '#f8f9fa', minHeight: '100vh', padding: '24px' }}>
      ...
      {/* Review Modal */}
      {showReview && selectedRes && (
        <div className="review-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowReview(false); }}>
          <div className={`review-modal-card${reviewSubmitted ? ' is-success' : ''}`}>
            <div className="review-modal-orbit orbit-one" />
            ...
            {reviewSubmitted ? (
              <div className="review-success-state" role="status">
                <div className="review-success-icon"><CheckCircle size={40} /></div>
                <span className="review-success-kicker">تم الإرسال بنجاح</span>
                <h3>شكراً لتقييمك!</h3>
                <p>مشاركتك تساعدنا على تحسين التجربة وتساعد المسافرين الآخرين على الاختيار بثقة.</p>
                <div className="review-success-rating" aria-label={`تقييمك ${reviewData.rating} من 5`}>
                  {[1, 2, 3, 4, 5].map((star) => <Star key={star} size={18} fill={star <= reviewData.rating ? 'currentColor' : 'none'} />)}
                </div>
                <button type="button" className="review-submit-button" onClick={() => setShowReview(false)}>العودة إلى حجوزاتي</button>
              </div>
            ) : (
              <>
                <p className="review-modal-description">كيف كانت تجربتك مع سيارة <strong>{selectedRes.make} {selectedRes.model}</strong>؟ حرّك المؤشر واختر التقييم الذي يعبّر عن تجربتك.</p>
                <form onSubmit={submitReview} className="review-form">
                  <div className="review-rating-panel">
                    <label>ما مدى رضاك عن التجربة؟</label>
                    <div className="review-stars" role="radiogroup" aria-label="التقييم من 1 إلى 5">
                      {[5, 4, 3, 2, 1].map((star) => (
                        <button key={star} type="button" className={`review-star-button${reviewData.rating >= star ? ' is-active' : ''}`} onClick={() => setReviewData({ ...reviewData, rating: star })} aria-label={`${star} نجوم`} aria-pressed={reviewData.rating === star}>
                          <Star size={34} fill={reviewData.rating >= star ? 'currentColor' : 'none'} />
                        </button>
                      ))}
                    </div>
                    <strong className="review-rating-label">{['', 'تجربة تحتاج تحسيناً', 'مقبولة', 'جيدة', 'ممتازة', 'استثنائية'][reviewData.rating]}</strong>
                  </div>
                  <div>
                    <label className="review-field-label">اكتب تعليقك <span>(اختياري)</span></label>
                    <textarea rows="4" className="review-textarea" placeholder="حدثنا عن نظافة السيارة، تعامل المورد، وسهولة الاستلام..." value={reviewData.comment} onChange={e => setReviewData({ ...reviewData, comment: e.target.value })} />
                  </div>
                  <button type="submit" disabled={submittingReview} className="review-submit-button">{submittingReview ? 'جاري الحفظ...' : 'نشر التقييم'}</button>
                </form>
              </>
            )}
          </div>
